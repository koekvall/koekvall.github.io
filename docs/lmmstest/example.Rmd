...
likelihood ratio, and Wald test statistics in a simulation. The theoretical
quantiles are from a chi-square distribution with 3 degrees of freedom. Data
were generated according to the model from the previous section assuming the
null hypothesis $(\sigma, \lambda_1, \lambda_2) = (1, 0, 0)$ (first plot), $(1,
0.01, 0.01)$ (second figure), or  $(1, 1, 1)$ (third plot). Circles correspond to
the proposed procedure, triangles to the likelihood ratio test, and plus signs
to the Wald test.
...
When no scale parameter is zero, classical theory says all three test
statistics are asymptotically chi-square distributed with 3 degrees of
freedom. The third plot indicates this approximation is good when
scale parameters are substantially different from zero. The second plot
shows the asymptotic distribution is a good approximation for the proposed
procedure but not for the other two when scale parameters are near zero.
